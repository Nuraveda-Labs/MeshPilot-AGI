...
class TestSheetTarget:
    def test_returns_sheet_when_configured(self, tmp_path, monkeypatch):
        from meshpilot import config as cfg
        from meshpilot.integrations.sheet_tracker import sheet_target
...
class TestAppendNewVideo:
    ...
    async def test_appends_row_with_parsed_fields(self, tmp_path, monkeypatch):
        from meshpilot import config as cfg
        from meshpilot.integrations import sheet_tracker
        ...
        async def fake_append(sheet_id, worksheet, columns, row):
            ...
            captured["row"] = row
        monkeypatch.setattr(
            "meshpilot.integrations.sheet_tracker.gs.append_row", fake_append
        )
    ...
    async def test_noop_when_sheet_not_configured(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "meshpilot.integrations.sheet_tracker.gs.append_row", must_not_run
        )
    ...
    async def test_swallows_api_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "meshpilot.integrations.sheet_tracker.gs.append_row", boom
        )
...
class TestUpdateByVideoName:
    ...
    async def test_delegates_to_update_row_by_key(self, tmp_path, monkeypatch):
        ...
        async def fake_update(sheet_id, worksheet, columns, *, key_column, key_value, updates):
            ...
            return True
        monkeypatch.setattr(
            "meshpilot.integrations.sheet_tracker.gs.update_row_by_key", fake_update
        )
...
class TestColLetter:
    ...
    def test_col_letter(self, n, letter):
        from meshpilot.integrations.google_sheets import _col_letter
