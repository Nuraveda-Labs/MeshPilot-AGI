...
Wiring contract (BIND-1b multi-brand pattern):
  ...
  - Per-brand tokens: `BRAIN_TOKEN_BSK_004_<BRAND_SLUG_UPPER>` (e.g.
    `BRAIN_TOKEN_BSK_004_ACME`, `BRAIN_TOKEN_BSK_004_PETCARE`).
...
Social is enrolled in `acme-supply` and `petcare` — both have
brain tokens in the consolidated `.env`.

Note on the brand-slug normalization: Social's model layer stores
brand_id in either snake form (`acme_supply`) or kebab form
(`petcare`). The normalizer accepts both; the env-key suffix is
always UPPER_SNAKE.
"""
...
def _slug_to_env_suffix(brand_id: str | None) -> str | None:
    ...
    Accepts both snake (`acme_supply`) and kebab (`urban-classics`)
    forms — both convert to UPPER_SNAKE (`ACME_SUPPLY`,
    `URBAN_CLASSICS`)."""
    if not brand_id:
        return None
